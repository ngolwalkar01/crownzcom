...
class Licensing {
	...
	public function activate_license() {
		...
		if ( ultc_filter_has_var( 'uo_reporting_license_activate', INPUT_POST ) ) {
			...
			$api_params = array(
				...
				'license'    => $license,
				'item_name'  => rawurlencode( $this->item_name ), // the name of our product in uo
				'url'        => home_url(),
			);
					...
					switch ( $license_data->error ) {
						...
						case 'expired':
							$message = sprintf(
								...
								date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) ) // phpcs:ignore WordPress.DateTime
							);
							break;
 ...
 $license_data->license = 'valid';
	...
	/**
	 * Show notice if shell_exec is not enabled
	 *
	 * @deprecated v4.1.3.1
	 */
	public function show_php_shell_exec_notice() {

		if ( 'manage-content' !== ultc_get_filter_var( 'page', '' ) ) {
			return;
		}

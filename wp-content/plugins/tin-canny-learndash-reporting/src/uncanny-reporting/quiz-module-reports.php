...
class QuizModuleReports extends Config {
	...
	public function user_quiz_report() {
			...
			foreach ( $user_courses as $course_id ) {
				...
				if ( ! empty( $quiz_attempts[ $course_id ] ) ) {
					...
					foreach ( $quiz_attempts[ $course_id ] as $k => $quiz_attempt ) {
						...
						$show_multiple_quiz_attempts = apply_filters( 'uo_tincanny_quiz_report_show_multiple_attempts', false, $course_id, $quiz_attempt['post']->ID );
		...
		$non_associated = $wpdb->get_results(
			$wpdb->prepare(
				'SELECT * FROM `%s` WHERE `user_id` = %d AND `course_id` = 0',
		...
		if ( $non_associated ) {
			...
			foreach ( $non_associated as $n_a ) {
				...
				$_quiz             = $n_a->post_id;
				$_statistic_ref_id = $wpdb->get_var( $wpdb->prepare( "SELECT activity_meta_value FROM `%s` WHERE activity_meta_key = 'statistic_ref_id' AND activity_id = %d", $_activity_meta_tbl, $activity_id ) );
				$pro_quizid        = $wpdb->get_var( $wpdb->prepare( "SELECT activity_meta_value FROM `%s` WHERE activity_meta_key = 'pro_quizid' AND activity_id = %d", $_activity_meta_tbl, $activity_id ) );
				$percentage        = $wpdb->get_var( $wpdb->prepare( "SELECT activity_meta_value FROM `%s` WHERE activity_meta_key = 'percentage' AND activity_id = %d", $_activity_meta_tbl, $activity_id ) );
				$modal_link        = '';
